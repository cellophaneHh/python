...
def greedy(items, max_weight, key_function):
    """假设items是列表，max_weight>=0 key_functions将物品元素映射为数值"""
...
def test_greedys(max_weight=20):
    ...
    test_greedy(items, max_weight, value)  # 按价值排序
    print('max_weight:', max_weight)
    test_greedy(items, max_weight, weight_inverse)  # 按重量排序
    print('max_weight:', max_weight)
    test_greedy(items, max_weight, density)  # 按占比排序
